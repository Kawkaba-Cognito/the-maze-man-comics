...
### The machine itself — what it is, and how it fails (2026-09-08)

**Lenovo, 8 GB RAM, 237 GB disk, Windows 11 Pro, and TWO GPUs**: an Intel UHD 620 integrated and an AMD Radeon RX 550 discrete (both on Lenovo subsystem `17AA`, so this is switchable graphics on one board, not an external dock). Work here is browser-driven and WebGL-heavy — Babylon, three.js in Home's universe and Void Runner, plus headless Chrome instances for verification — so it lives close to this machine's limits by nature.

⚠ **IT BLUE-SCREENED MID-SESSION WITH `INTERNAL_POWER_ERROR` (bugcheck `0xA0`).** Reported as *"my pc restarted because of power problem"* — it was not a power cut. `0xA0` is Windows faulting **during a power-state transition**, and on a switchable-graphics laptop the transition that matters is the discrete GPU being powered up and down as applications ask for it. What was running: **six or more headless Chrome profiles**, each with its own GPU process, driving WebGL scenes, for hours, on 8 GB. Both display drivers are old — **Intel `27.20.100.9749` dated June 2021, AMD `30.0.14030.0` dated January 2022**, four-plus years stale as of this note.

**Only one `0xA0` in ninety days**, so this is a load-triggered driver fault, not failing hardware. The practical rules that follow from it:

- **Run ONE headless Chrome, not six.** The previous session had accumulated profiles `cpA`, `cp8`, `cpLive2`, `cp-live`, `chrome-profile`, `chrome-profile2`. Reuse a single instance across checks.
- **Prefer software WebGL for verification**: `--use-gl=angle --use-angle=swiftshader --enable-unsafe-swiftshader`. It keeps the AMD driver out of the loop entirely, and for checking CSS grounds, layout and screenshots it is perfectly adequate. ⚠ It is **slower**, which breaks fixed-duration waits tuned against the GPU — that is what captured the Kawnera loading spinner and made a shipped fix look broken. **Wait on an element, not a clock.** And ⚠ it is not automatically the explanation for a visual oddity: the Void Runner band above was confirmed present on the real GPU too, so a difference must be *tested* against GPU rendering before being dismissed as an artifact.
- **Nothing was lost to the crash**, and that is worth knowing before panicking: every edit was already on disk in the working tree. What died was an in-flight verification sweep, which simply gets re-run.

⚠ **`C:\Windows\MEMORY.DMP` IS 7.7 GB AND THE DISK HAS ~16 GB FREE.** Crash dumps are set to `CrashDumpEnabled = 7` (automatic), which on this machine wrote essentially all of RAM. One crash therefore consumed nearly half the remaining free space — and a full disk is its own class of failure. The dump is safe to delete once a crash has been diagnosed; check for it after any BSOD. `Get-WinEvent -FilterHashtable @{LogName='System'; Id=41,1001,6008}` is the fastest read of what actually happened, and event `6008`'s *"previous shutdown was unexpected"* timestamp is a **stale checkpoint, not the crash time** — it read 3:37 PM for a crash at 4:22 PM. Trust the `1001` bugcheck record.

⚠ **OneDrive**: this repo lives under `OneDrive\Documents`, which is why `git stash -u` must never be used here (locks make it fail partway, deleting untracked work). See the memory note of the same name.
...
**A LEVEL THAT IS EIGHT BOARDS LONG AND EIGHT TIMES THE SAME BOARD IS NOT LONGER, IT IS REPEATED** (2026-09-18). Owner: *"make the levels longer, like multiple levels for each one, like waves. then i want to fix the difficulty system, for the waves."* The waves already existed — a Cancellation level has run 3 to 8 boards since 2026-09-17 — and they were **exactly flat**: `startLevelGame(lv, { continueWaves: true })` re-enters with the SAME `lv`, so every wave drew the same clock, the same interference and the same target count. Wave 8 of level 60 was statistically identical to wave 1, and difficulty varied between levels and not at all inside one.

A wave is a rung now. `fqLadderWaveTable` builds `[level][wave] = { tc, time }` once: targets climb `0.85 → 1.15×` the level's count while the clock falls `1.16 → 0.86×` its average, geometrically.

---

## Cancellation: the scientific rebuild (2026-09-19/20) — READ THIS BEFORE TOUCHING ITS DIFFICULTY

**`CANCELLATION-TASK-PLAN.md` at the repo root is the full record**, and its §0 is a handover. The project is **complete and deployed**; the two remaining items are parked for reasons given there. What follows is only what a reader of *this* file needs so the notes above do not mislead.

⚠ **`expertTargetSecForSetSize` IS SUPERSEDED. The model is now `expertTargetSecForBoard`.** The old one was a flat `700ms + 12ms × setSize/2`, and four things were wrong with it: the 700 ms baseline is 26% faster than the Mesulam rate its own comment cites (943 ms); it had **no motor term**, when Fitts on this game's own 52 px reference piece puts the tap alone at **270–310 ms**; the `/2` was an unnamed self-terminating-search assumption, so the stated 12 ms/item was really 6; and its slope was keyed to the **tier name** rather than to `interference` — the lever this game's difficulty actually turns on. The replacement prices search (on **N = targets + distractors**, with a slope that follows the same-hue share) separately from the tap (Fitts, Shannon form), on a **fixed reference device** so difficulty never becomes a property of the viewport.

⚠ **THE CLOCK IS DERIVED FROM THAT MODEL, SO FEASIBILITY IS STRUCTURAL.** `time / (per·tc)` reduces to the wave headroom, whose floor is above 1. `audit:fq`'s feasibility check is therefore a **gate**, not a report: a wave under 1.0× means the derivation broke, not that a number needs tuning. The mechanic multipliers (`drift` ×1.12, `dual` ×1.15) are priced **into the model, on the search term only** — they used to multiply the clock while `per` knew nothing about them, which banked and spent the same compensation at once.

⚠ **BOTH GATES NOW ASSERT DIFFICULTY, NOT GENEROSITY.** `audit:fq` and `audit:curves` read `fqLadderDifficultyLogit` — a Rasch `b` carrying clock tightness **and** absolute load. Seconds-per-target stopped describing difficulty the moment `per` began varying: a level can grant **more** spare time than the one before and still be harder, because there is more work in it (L38 grants 1.129× against L37's 1.042× and is plainly harder). `audit:curves` failed on exactly that and was right to.

⚠ **`b` NEEDS THE LOAD TERM OR IT GOES BLIND AT THE TOP.** Defined as generosity alone, every level from L40 up rated identically once the ratchet drove them onto the feasibility floor — twenty levels called equally hard while the board went 35→48 cells and the same-hue field 83%→94%.

⚠ **`audit:pacing` COVERS `cancel-task` NOW** (it never did, despite the description above implying it). The unit is **ms per board ITEM**, not per target — `audit:fq` already gates per-target pace; this gates whether there is time to look at the board at all.

⚠ **THE 3 s WRONG-TAP PENALTY IS A NAMED CONSTANT** (`FQ_WRONG_TAP_PENALTY_SEC`), and the forbidden object costs **+2 s** on top (`FQ_NOGO_EXTRA_PENALTY_SEC`) — band 4's rule used to be a HUD chip over unchanged behaviour. The cost is **time, not an error multiplier**, because error counts are the least reliable thing this task produces (minimal detectable change is 135–219% of the mean error count).

⚠ **`lookalikes` IS NOW `samehue`.** It declared a mechanic no code read, and `audit:fq` actively *forbids* the same-motif pools that would have implemented it. What band 5 does is real and is the same construct in another feature: 89–94% of distractors wear the target's colour.

⚠ **"Why this trains your brain" IS GONE FROM ALL 18 GAMES** — it was the headline over a panel whose own closing section says broad transfer is debated. `HubScienceLink.jsx` says "The science" / «العلم وراء اللعبة».

⚠ **A Rasch/Elo ability (θ) now exists** (`shared/abilityElo.js`, pure maths, gate-loadable). It places Survival boards at the 0.84 target, suggests a start level, triggers the coach after a failed level with a sub-50% predicted clear, and is what the attention rating banks — **it advises and never gates**. Only the binary outcome feeds it; d′, ICV, CP and the organisation indices are the *instrument* and must never drive the controller.

⚠ **THE BETWEEN-LEVEL ENDPOINTS HAD TO MOVE UP TO PAY FOR IT.** They were 2.35 → 1.06× expert, which left nothing to spend inside a level — at 1.06 the level average was already almost on the floor, so any wave tighter than average would have gone under 1.0, the one line this game must not cross. At 2.60 → 1.22 those numbers are the level AVERAGE, and the hardest board a level deals is tighter than the old flat value while the level opens gentler. The felt ceiling goes down, not up.

⚠ **THE MECHANIC MULTIPLIERS ARE BAKED INTO THE TABLE, NOT APPLIED DOWNSTREAM.** `drift` and `dual` each buy time back and used to be multiplied on in `prepareLevelRound` **after** the table had capped each level against the last — so the cap was computed on a number the player never saw, and "never more generous than the level before" was false on six of the fifty-nine steps. A caller-supplied `tlimSec` is now final and the multipliers are not re-applied to it. Same family as the `ABSOLUTE_TIME_FLOOR_SEC` clamp that used to sit two lines away and silently override the caller's floor.

⚠ **AND THE CAP IS ON THE LAST WAVE**, because that is the level's true difficulty. Capping the average would let a level with more waves sneak an easier finish past a level with fewer.

⚠ **`opts.tcOverride` IS THE ONLY WAY THE RAMP REACHES THE BOARD, and without it the whole table is dead code** — `tc` came from the curriculum via `reflowTargetCount` and no caller could move it. The game and `audit:fq` now both build a wave through **one** function, `fqLadderRoundOpts(lv, waveIdx)`, so a gate cannot certify a board nobody is dealt.

⚠ **THE 60 LEVELS A PLAYER ACTUALLY CLIMBS WERE NOT FEASIBILITY-GATED AT ALL.** `audit:fq`'s loops walk the authored curriculum (3 tiers × 100) and Survival; the ladder is a PATH through that curriculum with its own clock, and `audit:curves` only checks that the path climbs, never that a rung can be finished. So the one property this game exists to guarantee was unasserted on the mode most players open. It now builds and measures **990 wave-boards** — feasibility, tappability, the dealt round matching what the table asked for, the ramp inside each level, and the last wave never easing off. Verified by planting four bugs (a dead ramp, an unwinnable last wave, a reversed target climb, a removed cross-level cap); each fires, and each plant was confirmed to have landed in the file before the run — this tree is CRLF, and a regex plant that silently fails produces a false PASS.

⚠ **A DUAL ROUND HAS TWO TARGET OBJECTS AND THE AUDIT'S BOARD CHECK KNEW ABOUT ONE.** It never fired because nothing in the file had ever built a dual board — the curriculum does not carry the rule, and the ladder, the only path that turns it on, was the unaudited half. The first run of the new gate failed every level from L21 for a board that was exactly right.

⚠ **THE IN-PLAY HUD WAS NAMING THE WRONG LEVEL.** `round.lv` is the authored level `prepareLevelRound` was given, which on the ladder is a tier index — so tapping level 5 on the map opened a board whose HUD read **L23**, and level 21 read **L1** directly after level 20 read **L100**. It reads `L{ladderLv}` with the wave beneath it now. ⚠ Two lines by measurement, not by hope: the chip is **51px** wide in 16.8px DM Mono, so `L1 · 1/3` (81px) wrapped — and it broke at the separator, leaving a lone middle dot on a line of its own.

⚠ **`content-visibility: auto` BROUGHT PAINT CONTAINMENT AND SLICED THE MAP'S CAPTION IN HALF.** `.cpp-node` uses it so the other 59 nodes of a 6,610px path skip rendering; it also clips each node's painting to its own 92/112px box, and `.cpp-sub` is 169px. The chip rendered as a square-cornered bar reading "argets · 6s · 3 wav". Nothing in the CSS draws a rectangle, the element measured its full width, and every ancestor was `overflow: visible` — reading styles finds nothing, only a screenshot shows it. The current node opts out (`content-visibility: visible`), which costs nothing because the map scrolls to it on open. Third entry in this file for the same property's containment.
...
**BUTTONS BEAT A BOARD YOU TAP, AND THE PROOF HAD TO CHANGE SHAPE WITH THEM** (2026-09-05). Intercept's input became three weapon buttons; the board is now a picture (`.ic-field` lost its `cursor: pointer` — promising a target that does nothing is the same lie as a blank button label). Asked for by the owner, and it measures *better*: tapping a small moving target confounds reaction time with **aiming** (Fitts), while a fixed button holds the motor act constant and two or three of them make it **choice** RT with a mapping to learn.

⚠ **The feasibility proof stopped being a scheduling proof and became a COVERING one.** A press hits everything in that weapon's stretch (waves got easier — a tap-per-marcher proof would now reject waves a player clears comfortably) while each weapon has its own reload, far longer than the old 240ms thumb gap (waves got harder). `feasible()` now serves the most urgent marcher at the latest moment that still works, with one step of lookahead so a late shot never blocks the next marcher that weapon owes. Getting there cost four measured failures, all of which read as "this level is unfair" rather than as bugs: serving an unbound marcher through the wrong weapon (**65 of 300** waves unclearable), the lookahead forgetting *my own* second shot (armour), the turret's authored 520ms reload overrunning the dwell at **exactly one level of a hundred**, and two turret-bound armoured marchers back to back — removed structurally by never binding armour to a weapon that cannot kill it in one shot.

⚠ **RELOAD AND BLAST BOTH YIELD TO GEOMETRY, like `ringSpan` yields to the dwell floor.** `COOL_DWELL_MAX` / `COOL_GAP_MAX` cap a reload against the dwell and the column's pace (a non-heavy weapon gets *half* the dwell — it needs two hits inside one window), and the mortar's blast is clamped against the real gap to its neighbour, because the stretches shrink with the curve while the centres stay put. An authored 0.03 that is clear at L100 reaches into the turret's ground at L81, and `boundOf` rests on those stretches staying disjoint.

⚠ **THE OLD SELF-TEST STOPPED BEING A TEST**, which is the most important thing in `validate:intercept`. Its planted "impossible" wave was three marchers sharing one 100ms window — now cleared by a single press. The plant would have passed, the gate would have said OK, and nobody would have learned that `feasible` had stopped checking anything. What is unclearable now is *separation*: windows that do not overlap, closer than the reload.

⚠ **AND ONE ASSERTION WAS SIMPLY WRONG.** The first version required a shell's flight to fit inside the dwell and failed L84 for a mortar that "could never land on anyone". Flight time is a **lead**, not a window: you press before they arrive. Measure a constraint before asserting it — a gate that fails working code gets weakened, and then it protects nothing.

⚠ **A WAVE IS NOT CLEARABLE IF CLEARING IT REQUIRES SHOOTING A FRIENDLY** (2026-09-05). Reported by the owner as *"when you hit the turret all the balls inside the area zone are attacked, so when there is a red ball it will be shot also"*. Structural, not tuning, and it had been true of most of the game: **90.7%** of no-go marchers stood in a weapon's stretch beside a threat at some point, and on **85.9%** of waves the OPTIMAL clearing plan killed at least one friendly. So the commission count — the whole inhibition measure — was mostly recording waves where withholding was impossible, and the player was shown **DON'T HIT** over a shot they had to take.

One line caused it: `feasible` opened with `units.filter(u => u.kind !== KIND.NOGO)`. It proved a wave clearable and never asked at what price, so nothing in the build could see it.

⚠ **THE FIX IS NOT TO STOP THE SPLASH, AND NOT TO SPARE FRIENDLIES FROM IT.** A press must serve everything in the stretch — that is what makes a crowded wave clearable and what keeps aiming out of a game that deliberately removed it. And if a press can never hurt a friendly there is nothing left to withhold, so the measure dies outright. What is required is that a **clean line exists and the player has to find it**: overlap is still allowed and is the point, because the prepotent response is to fire the moment a target is in range and the player must hold that press while a friendly crosses. Now: `feasible` subtracts the friendlies' windows as forbidden intervals (`cleanStarts`/`pickClean`), and `buildWave` re-deals until the result passes — **81% are clean on the first deal, the worst measured case needed 8, none exhausted the loop** across the ladder plus survival to stage 90. Verified after: 6,660 waves, 76,620 friendlies, **zero** forced collateral.

⚠ **BOTH OF ARMOUR'S SHOTS MUST LAND CLEAN, NOT JUST THE FIRST** — `cleanStarts` shifts each forbidden interval back by k reloads, once per hit. The same family as the lookahead that forgot its own second shot. It has its own plant, because a first-hit-only check passes every other test in the file.

⚠ **AND `feasible` MUST SAY WHICH FAILURE IT IS.** Its only reason used to be "still reloading", which would have reported a friendly conflict as a cooldown problem and sent the next reader to tune `coolMs` — a repair that cannot work. The two want opposite fixes: a reload problem is a weapon spec, a friendly problem is the column's spacing.

**A GENERATED GAME CAN BE ALL DIFFERENT AND FEEL ALL THE SAME** (2026-09-05). Reported by the owner: *"make it more clear, like a story, and make it not all the same — you are repeating the same stories."* Detective's **logic** was never repetitive (12 statement types × 6 rules × 7 questions, and the line-up was already a fresh shuffle of the cast). What repeated was the **fiction, because there wasn't any**: a case knew only `{ thief, truth }`, so every one was "somebody took the unnamed thing". Now every case names a place and a missing object (`SCENES` in `data.js`) and the statements are phrased about it, three wordings per kind, dealt with the case so the same seed gives the same sentence.

⚠ **THE LOGIC ENGINE DOES NOT KNOW THE SCENE EXISTS**, and that is what made this safe to add to a game that was already correct. `validate:liars` re-solves every case twice — as dealt, and with the scene stripped — and requires an identical answer. It also renders every kind against every scene in both languages, because a place × object × phrasing cross-product is exactly the shape that ships "Broken Lion".

⚠ **Three gate bugs, and only plants found them.** `nameOf('noor')` returned `''` because Noor's id is `mimi`, and `includes('')` is true for every sentence — 180 false failures. A plant for an English Arabic half went **uncaught** because the check only looked for *some* Arabic and the substituted count word supplied it; the rule that holds is "no Latin letters at all". And the one bug no gate could have found was found by reading the screen: *"Search me. the good knife is not mine to take."* — a scene word opening a sentence. There is a rule for it now.
...
## Wellbeing — "The Quiet Observatory" (2026-09-06)

**`src/features/relax` HAD NEVER BEEN SCANNED BY ANY GATE, AND THAT IS THE WHOLE STORY.** `audit:design` walked `src/features/training/domains` and nothing else, so Wellbeing had no ratchet, no ceiling and no signal — and had accumulated **596 raw colour literals against ~60 token references**, in a fifth of the code the training tree occupies. It was not carelessness; nothing could tell anyone it was happening. `EXTRA_DIRS` in `scripts/audit-design.mjs` now walks it too. ⚠ Verified by planting a `#ff00ff` in `PracticeShell.jsx` and watching the gate fail `580 → 581`.

⚠ **RULES 2 AND 3 ARE NOW GAME-SCOPED, because they are claims about a PLAY SURFACE.** Unscoped, `foreign-surface` reported four findings against `RelaxScreen` for using `UniverseStage` — which is the app's shared starfield chrome that Home wears too, and is exactly right on a landing whose entire design is a sky. The only ways out would have been to baseline a false positive or to take the sky off. **A gate that fails correct code gets weakened, and then it protects nothing.** Re-verified after scoping by planting `--play-surface-deep` in `intercept.css`: still fires.

⚠ **THREE FILES EACH DECLARED THE SAME SIX COLOUR NAMES IN FROZEN HEX** — `PracticeShell` exported `INK/SUB/FAINT/LINE/CARD/GOLD`, `quizShared` imported them, and `DailyHabits` re-declared its own identical set. Eight files interpolate them into CSS template strings, so every one emitted a literal no theme could reach — and the dark appearance therefore existed as a **second hand-written palette** under `[data-home-theme='dark']` (nine overrides in `PracticeShell`, seventeen in `quizShared`, fifty-seven in `DailyHabits`). Two palettes, nothing forcing them to agree. Pointing the names at `var(--rx-*)` let the duplicates in `PracticeShell` and `quizShared` be **deleted outright** — the dark practice screen now themes itself, verified on screen. ⚠ `DailyHabits`' block is deliberately still there: `global.css` owns that screen's dark ground through an `html[data-home-theme='dark'] .rx-root` rule carrying `!important`, and its overrides are written around that interaction. Remaining debt, now capped.

⚠ **`--color-training-palette-surface` WAS THE PRACTICE-SCREEN GROUND.** tokens.css says outright the training palette is the one thing never flipped for theme, so every practice sat on a fixed pale board while the chrome around it went dark. Wellbeing derives from `--universe-*` now, which is the family that actually flips. CLAUDE.md already recorded this same mistake on the Habits screen.

⚠ **THE LANDING IS OUT OF SCOPE — THE WORK IS INSIDE THE PLANETS** (owner, 2026-09-06). A first pass also rebuilt the constellation: shaded spheres carrying each area's hue, and Dr Kawkab at the foot of the sky speaking the hint. **That was reverted at the owner's instruction** — the outer Wellbeing design stays as it is, and this work belongs to the category screens and the practices behind them. `RelaxScreen`'s landing markup and CSS are byte-identical to before; only `detailHeader`, the category root's hue and `.rx-cat-planet` differ, all of which are *inside*. ⚠ **Do not re-apply the landing redesign.** The reverted rules (`.rx-guide*`) were deleted from `wellbeing.css` rather than left unused, because a dead selector in a palette file is an invitation to wire it back up.

⚠ **THE CHARACTER WAS A DIFFERENT CHARACTER, IN TWO PLACES — and that fix stands, because it was wrongness rather than absence.** `quizShared` rendered `CosmosCharacter faceOnly` — `kawkab-face.png`, the retired sprite — as a cropped head peeking over a bubble reading *"Hi, I'm Kawkab!"*, while the Training hub's Kawkab is `kawkab-planet.webp`. `KawkabSprite`'s own header already documented this ("CosmosCharacter — 2D, but a different character"); Wellbeing was never included in that sweep. And `habitReminders.js` shipped **`guide-fox-sprite.webp`** — a fox — as the notification icon, which is the one place the mascot appears *outside* the app. Both are the shared `KawkabSprite` now. Both are inside the planets; the constellation is not.

⚠ **THE PER-AREA HUE WAS DEFINED, WIRED AND INVISIBLE, and it still is on the landing.** Each category carries a `color` painted as `${color}42` over `${color}20` — 26% then 12% alpha — behind a **Fluent emoji**, so all five orbs render the same grey-lilac disc and only the caption tells Sleep from Relationships. That is the landing's original behaviour and is left alone. `wellbeing.css` gives each area a `core`/`lit` pair used **inside**: the category header's `.rx-body` and `PracticeHero`. ⚠ The underlying inequality is worth recording either way: `planetIcons.js` gives the six **training domains** hand-painted `category-drawings-2026` art and gives the five **wellbeing pillars** emoji. Five paintings is the right long-term fix and is the owner's call.

⚠ **`display: block` ON `.rx-body` IS LOAD-BEARING, and its absence fails invisibly.** It is a `<span>`, so it is inline by default — width/height are ignored and it measures **0×0 while `getComputedStyle` still reports the full gradient**. The landing hid this completely, because its orbs sit in a flex container and a flex *item* is blockified automatically; the category header is a plain block, so it reserved 40px of layout and painted nothing. Found with `getBoundingClientRect`, not by reading the CSS.

⚠ **`align-self: center` ON `.rxp-primary`.** `.rxp-body` is a flex column with default `align-items: stretch`, and a stretched item that caps itself with `max-width` goes flush to the inline start — so every practice's Begin button sat 340px hard-left in a 480px column while the chips above ran full width. Same bug as `.ct-play-results-actions`, different container.

## Wellbeing — the clinical content audit (2026-09-07)

Audited as a clinical psychologist would, then fixed. The writing was never the problem — it was already non-pathologising and cited. What it carried were **three overclaims, two psychometric bugs that biased every result in one direction, and no safety layer at all.**

⚠ **THE FEATURE HAD NO CRISIS ROUTE — ONE SENTENCE, BURIED IN THE MBSR GUIDE TAB.** The landing had none, neither quiz had one, and Grounding — sold explicitly for *acute anxiety* — had none. A person could answer twelve items about being abandoned, be labelled **Fearful-Avoidant**, read that it traces to a caregiver who was also a source of fear, and reach a "Retake the quiz" button with nothing else offered. `SafetyNote.jsx` is now on **every category screen** (each of the five is the only way to reach a practice), on both quiz results, on all three measured practices, and in the MBSR guide. ⚠ **No phone number is hard-coded and none may be added** — this ships EN/AR to several countries, and a US three-digit line is worse than absent because it looks like provision. It links a directory that resolves by country; "your local emergency number" is the one universal instruction, so it is the one given as an instruction.

⚠ **THE ATTACHMENT QUIZ HAD ALL TWELVE ITEMS KEYED THE SAME WAY**, so agreeing always pushed a score up. That handed the result to acquiescence: a yes-biased respondent scored high on anxiety **and** avoidance, and the classifier then cut at the raw scale **midpoint** (4.0) — two independent errors pushing the same people into the same corner, the rarest and most alarming of the four labels, on a screen that then told them most adults are secure. Half the items are reverse-keyed now. **Verified by driving the real app**: all-7s used to pin both axes, and now gives anxiety 5.0 / avoidance 4.0.

⚠ **AND THE CUT POINTS ARE NOT THE COMMUNITY MEANS EITHER — that is a median split**, which sorts ~25% of users into each box and contradicts the "half to two-thirds are secure" line three paragraphs below it. They sit near the upper quartile instead, so low/low ≈ the ~56–60% the screen actually reports. ⚠ **The avoidance cut is above 4.0 for a STRUCTURAL reason**: a balanced 3-direct/3-reverse subscale puts a uniform response set — every item answered 4, i.e. "I don't know" — on **exactly 4.0** by construction, so any cut below that labels the undecided user avoidant every time. Measured at 3.9: all-4s, all-7s *and* all-1s each came out avoidant. Now `{anxiety: 4.5, avoidance: 4.2}`, and anything within 0.5 of a cut renders a `borderline` caveat saying the label is a coin toss and the two numbers are the real result.

⚠ **THE QUADRANT'S CROSSHAIRS MUST DERIVE FROM `CUT`.** They were hard-coded to 50%, which agreed with a midpoint classifier *by coincidence* and stopped agreeing the moment the cuts moved. A dot in the bottom-left under a caption reading "Dismissive-Avoidant" is a contradiction a user notices and cannot explain, and it discredits the whole result.

⚠ **THE BIG FIVE COMPARED SCORES TO 50, AND 50 IS NOT AVERAGE.** `buildTakeaway` ranked traits by distance from the bar's midpoint, but published TIPI means sit well above it (agreeableness ~5.2/7, conscientiousness ~5.4/7) — so "your two most distinctive traits" selected the same two for nearly everyone, and an average-agreeable person was told warmth defines them. Comparison is against `NORMS` now. ⚠ And the bar shows a **band**, not the bare integer: two items per trait carries too much error for a point estimate, and a retake moving several points on noise reads as a personality change.

⚠ **THREE OVERCLAIMS, ALL IN THE LABEL RATHER THAN THE BODY.** (1) The subtitle said *"A validated 12-question quiz (ECR-S)"* while the file's own comment said the items were adapted and reverse-scoring deliberately dropped. (2) **"8-Week MBSR" is not MBSR** — that is a manualised taught course (eight ~2.5h classes, a certified teacher, group inquiry, a day-long retreat between weeks 6 and 7), and the evidence people mean by the word was collected on the taught format. It is "8 Weeks of Mindfulness, inspired by MBSR" now; **the `id` stays `'mbsr'`** so saved progress and habit links survive. (3) The **"measurable brain changes"** clause is gone and must not return — it alluded to the 2011 hippocampal grey-matter finding, which failed to replicate in a well-powered active-controlled RCT (Kral et al., 2022). What replaced it is stronger *because* it is qualified (Goyal et al., 2014).

⚠ **CITATIONS WERE CHECKED ONE BY ONE, AND FOUR WERE WRONG.** `r ≈ .31` attributed to Barrick & Mount (1991) — that meta reported ρ ≈ .22, and Sackett et al. (2022) argue the whole family was overstated, so the figure is gone rather than corrected. The "avoidants report calm while the body shows stress" finding is **Dozier & Kobak (1992)**, not Fraley & Shaver (1997) (the airport study). Eysenck's arousal theory was stated twice as settled mechanism. And fearful-avoidant was described as *"the least common and most researched of the four"* — self-contradictory in one sentence, and false.

⚠ **PRACTICES ASSERTED OUTCOMES THEY COULD NOT KNOW.** Grounding ended on "Notice how everything slowed down a little", PMR on "Your body is calmer now". Telling a distressed person their distress fell when it did not is invalidating, and it teaches them the tool does not work for them. Both now ask — a 0–10 rating before and after (`PracticeCheckIn.jsx`, SUDS) — and report the number the user gave. ⚠ **The "it got worse" branch is not an error path**: relaxation-induced anxiety is common and likeliest in exactly these users, so that branch is written first and reads warmest.

⚠ **`practiceLog.js` IS GAMIFIED ON THE SKILL, NEVER ON THE MOOD SCORE.** The moment "calmer" pays better than "still wound up", the instrument stops measuring — users learn to report the number that wins. So tiers count **sessions done**, there is no streak and no decay, and a session where distress rose is kept and shown as useful. What is rewarded is practice and discovery: skill tiers, a personal best drop, and `getSignatureMove()` — which practice actually settles *this* person — **gated at two rated sessions on each of two practices**, because an n-of-1 comparison from single sessions is noise presented as a personal insight.

⚠ **A SKIPPED RATING IS `null`, NOT 4.** The old attachment scorer did `answers[it.id] || 4`, inventing a midpoint response for anything missing. The check-in must never do that: skip is always offered (this screen stands between a distressed person and the tool they came for), and a null simply never enters an average.

⚠ **THE ROUTER LIVES INSIDE THE CATEGORY SCREENS, NOT ON THE LANDING** (owner, 2026-09-07, re-confirming the 2026-09-06 rule). `NeedStrip` renders only where an area holds ≥2 distinct answers — in practice Stress & Calm and Sleep. ⚠ **A chip does not open the practice directly**: it reveals *why* that tool suits that state, because a router that silently teleports you teaches nothing and must be re-consulted forever. ⚠ And the mapping is mechanism, not taste — **panic routes to Grounding, deliberately not Breathe**, since breath-focus mid-panic can amplify the interoceptive cues the panic is feeding on. The `flat` state routes to Meaning and says outright that no calming tool lifts a flat mood in three minutes; a router that always has an answer is one nobody believes twice.

⚠ **BREATHING AND PMR ARE NOT INERT, AND SHIPPED WITH NOTHING BUT "not medical treatment"** — which says who is not responsible rather than what to watch for. 4-7-8 is a 19-second cycle with a long hold; the flat 5-minute option served ~16 cycles against Weil's own "start with four", on the one pattern most able to reproduce the light-headedness an anxious user already fears. `maxMin` caps it at 2 minutes, **per-pattern** — coherent breathing genuinely wants five, being a dose-dependent vagal lever. PMR now says tense to ~70%, never to pain, skip injured areas, and stop if it feels worse.

⚠ **REWRITING A STEP LIST DROPPED A NOUN AND NO GATE SAW IT.** Folding "things you" into each step's label produced **"Name 5 you can see"** on the live board — found by driving the app, not by lint, build or any of the fourteen gates. The count word inflects in both languages (`things`/`thing1`, أشياء/شيئاً واحداً) and the Arabic taste step agrees with the *singular* noun (تتذوّقه, not تتذوّقها). Same family as the audit's other finding: `habitState.js` shipped `titleAr: 'لحظة quality'`, a Latin word inside an Arabic label, and the Habits disclaimer said "not medical advice" in English and "ليس تشخيصاً" (*not a diagnosis*) in Arabic — a different claim, in the one string whose whole job is to be the claim.

### The features that closed those gaps (2026-09-07, same day)

**`who5` — the WHO-5 Well-Being Index.** The one validated OUTCOME measure in the feature; before it, nothing could answer "is any of this working?". ⚠ **It is presented as a wellbeing score and never as a depression screen.** The published <50 cut-off is used, but it triggers an *invitation* to talk to someone — never a label, never a probability, never the word applied to the user. The Assessment is parked behind "coming soon" for exactly this reason; WHO-5 ships because it is finished, validated (Topp et al., 2015 systematic review) and used only for what it is validated to do. ⚠ **The 14-day window is enforced, not decorative**: every item asks about "the last two weeks", so re-taking is gated to 7 days. That gate is also the only thing between this and score-chasing — a wellbeing number you can re-roll on demand is a slot machine.

**`sleep-reset` — a sleep diary + stimulus control.** Sleep shipped as relaxation, breathing and a rain loop: the three *weakest* arms, while CBT-I — first-line in every major guideline, Trauer et al. (2015) — was absent. ⚠ **SLEEP RESTRICTION IS DELIBERATELY NOT IMPLEMENTED and must not be added without a clinician.** It works by inducing short-term sleep deprivation: it can precipitate mania in bipolar disorder, lowers the seizure threshold in epilepsy, is wrong with untreated apnoea, and degrades alertness for a week or two. The screen explains it and stops. What it offers instead is the **anchored rise time**, the safest strong lever. ⚠ It also **screens once for apnoea** (snoring/gasping/daytime sleep attacks) and routes to a doctor — CBT-I is the wrong tool there and delay is the harm. ⚠ `computeNight` rejects a night whose final wake falls outside the time in bed: a plain typo produced an arithmetically valid 15-minute night at 0%, saved it, and dragged down the average that the whole programme turns on.

**`connect` — Good News (capitalization).** Relationships held one quiz *about yourself* and no practice — a pillar you could finish without doing anything involving another person. Teaches active-constructive responding (Gable et al., 2004) across four scenarios, then ends on a real action. ⚠ **Its evidence tier is stated honestly in-app as a replicated research programme, NOT a meta-analysis** — overstating it would make the genuinely strong claims elsewhere (CBT-I, implementation intentions) less believable.

**SRBAI replaces the 66-day bar.** `FORMATION_DAYS = 66` was the **median** of an 18–254 day range (Lally et al., 2010), shown as a target — and a day count was standing in for automaticity, which is a different claim from the one "Stable" makes to a reader. `getAutomaticity` now returns `measured: true|false`, preferring a real four-item SRBAI rating (Gardner et al., 2012). ⚠ **`normalizeState` is a WHITELIST** — a key it does not rebuild is dropped on every load, silently; `automaticity` had to be added there or every rating would have vanished at next launch with no error.

**Attribution, in the Reflect tab.** "Habits are votes", "never miss twice" and stacking are James Clear; starting tiny is BJ Fogg — all previously uncredited. Meanwhile the one mechanic here with real evidence, the if-then anchor (**implementation intentions**, Gollwitzer & Sheeran 2006 meta-analysis over 90+ studies), was never named. The app was borrowing others' words while giving away its own strongest claim.

⚠ **QUICK COMES BEFORE PROGRAMS, AND IS THE DEFAULT** (owner, 2026-09-07). Programs led, so opening Stress & Calm showed one card — the 8-week programme — while Breathe, Grounding and the check-in sat behind a tab nobody had a reason to press. Someone opening Wellbeing while wound up wants the two-minute thing.
...
### Superseded the next day — ONE BACKGROUND FOR THE WHOLE APP (2026-09-08)

The owner's model, stated directly: *"the background should be applied on the whole app, one for everything except the gameplay, and one while playing either training or puzzle."* Built that way — `--app-bg` on `body`, `--game-bg` on `.ct-domain-game-stage::before` and the puzzle/Void Runner play roots, each switching light/dark × landscape/portrait from `app-backgrounds-v2/` and `gameplay-backgrounds-v2/`. **Then the two-image half was reverted**, `--game-bg` briefly derived from `--app-bg` — **and on 2026-09-08 the owner reversed that again after seeing both on a phone**: *"use the minimalist background and set it as the background for all the games gameplay"*. `--game-bg` names the `gameplay-backgrounds-v2/` pair once more. See the batch at the end of this section for the current state.

⚠ **THE SECOND IMAGE WAS ABANDONED ON EVIDENCE, NOT TASTE.** Reported as *"why cancellation task doesnt have any background, maybe there are other games that dont have"* — and the report was fair, but the diagnosis everyone reaches for is wrong. Codex composed the gameplay art with a few small planets in the **corners** and an empty centre; `background-size: cover` crops the sides on a phone, so it trimmed the only content it had. The layer was working perfectly. There was nothing on it.

⚠ **DO NOT GO LOOKING FOR A COMPOSITING BUG HERE.** Three checks proved the plumbing sound before the art was blamed: forcing `.ct-domain-game-stage::before` to solid magenta painted the **entire** screen (both with and without a `z-index` override); the image returned 200 and was present in `dist/`; and a sweep of all six domains found the art present on **every** game. An earlier note in this file claiming the art "does not composite during a live training round" was simply wrong and has been removed.

⚠ **A COMPUTED `background-image` PROVES THE DECLARATION AND NOTHING ELSE.** The verification harness read computed style across 4 viewport × theme combinations and reported green while the screen looked blank — the art was declared, loadable, and empty. Same family as the blank-label bug that passed a build, a lint and nine gates. **Screenshot it.** `Page.captureScreenshot` over CDP is two lines and it is the only check that would have caught this.

⚠ **A RICH-MENUS / QUIET-BOARD SPLIT WAS TRIED AND REJECTED ON MEASUREMENT.** The obvious refinement is `:has(.ct-training-play-header)` — quiet art only while a round is live. Measured across three games, that header is **already present on the MENU** in Target Tracking and Story Time (absent only in Cancellation), so it would mislabel most menus as gameplay. There is no reliable "a round is live" marker today; if one is ever added, `--game-bg` is the seam that is deliberately still named.

⚠ **`--game-bg` IS RE-DECLARED AGAIN (owner's call, same day), AND ADJACENCY IS WHAT REPLACED THE SAFETY.** Deriving it made a half-migrated pair inexpressible, which is why it was done; naming a second image brings that risk back in its usual shape — four parallel declarations (light/dark × landscape/portrait), one retargeted and its partner left stale, a failure this file records five times in one day. The defence now is **physical**: each `--game-bg` line sits **immediately after** its `--app-bg` partner in `global.css`, so retargeting one puts the other on the next line down. Do not separate them.

⚠ **THE "EMPTY CENTRE" FINDING STILL STANDS — it was overruled, not disproved.** The minimal art really is nearly empty in the middle and `cover` really does crop the sides on a phone. The owner chose quiet over visible for the one surface a timed task is played on, which is the same reasoning that produced a second image in the first place. So if anyone reports a missing gameplay background again, the answer is almost certainly *"the picture is empty"*, not *"the CSS is wrong"*.

⚠ **THE PER-DOMAIN ART MOVED TO `::after`, AND `::before` IS A TRAP.** `.ct-domain-pick::before` is the domain screen's pre-existing decorative glow, owned by three other rules. Putting the domain art there measured `domain art: none` in **dark** while reading correctly in light: two dark rules (`training.css:7303`, `:8211`) set the `background` **shorthand**, which resets `background-image` to `none` — a shorthand does not have to mention a longhand to erase it. And the light rule at `:9301` applies `opacity: 0.34` + `mix-blend-mode: multiply`, right for a glow and wrong for a painting, so even the appearance that "worked" showed the art at a third strength. `::after` is used by nothing on that screen.

⚠ **THE WELLBEING LANDING NEEDED (0,3,1) TO GO TRANSPARENT.** `.rx-root--landing` alone is (0,1,0) and lost to RelaxScreen's injected `html[data-home-theme='dark'] .rx-root { background: … !important }` at (0,2,1) — two `!important`s, so specificity decided it, and the landing covered the app background **in dark only**. Matching that specificity would still be a coin toss on file order against an injected `<style>`, so the rule is deliberately over-specific.

⚠ **Home needed the 3D canvas made transparent** — `ZenUniverse`'s canvas was already `alpha: true` with `setClearColor(…, 0)`; the blocker was the **root `<div>`**'s opaque `background`. The particles still render.

Verified after every change: 4 viewport × theme combinations, all five tabs unblocked, correct image variant on body / domain / in-game, zero exceptions — **plus screenshots**.
...
The owner played the shipped build on a phone and reported six things in a row. Five were cosmetic. One was not, and it had been true of every game on the platform for as long as the stage has existed.

⚠ **`overflow: hidden` ON `.ct-domain-game-stage` CLIPPED EVERY TALL GAME, WITH NO WAY TO REACH THE REST** — *"there is a sliding problem … like detective i cant go down"*. That element wraps **all 18 games** launched from Training or the Daily Workout, and it is `position: absolute; inset: 0` — exactly the viewport, never taller. Anything past the fold was not merely off-screen, it was unreachable: no scrollbar, no wheel, no touch drag. Measured on a 1366×577 desktop, Detective had **295px** below the fold holding its three statements, the jail and the submit button — so the game could be started and could not be finished. The clip applied equally to all eighteen; it only *bit* the tall ones, which is why it survived this long.

`overflow-y: auto` changes nothing for a game that fits and gives the tall ones a way down; `overflow-x` stays hidden so a wide board still cannot shove the page sideways. ⚠ **`overscroll-behavior: contain` is not decoration** — without it a scroll reaching the end of the stage chains to the page behind and drags the whole app under a live round.

⚠ **AND MAKING IT SCROLLABLE MOVED THE ART.** `.ct-domain-game-stage::before` was `position: absolute`; an absolutely positioned child of a **scroll container** is laid out against the padding box and scrolls with the content, so on a tall game the background would slide up and leave a flat band under it. It is `fixed` now. It resolves against `#ui-shell` (a transformed ancestor traps `fixed`), which is the app area, so the framing is unchanged. **Whenever you make a container scrollable, re-check every absolutely positioned child it owns.**

⚠ **THE PUZZLE STUDIO PAINTED ITS OWN GROUND, WHICH IS WHERE THE APP BACKGROUND STOPPED** — *"you didnt put the backgrounds in the whole studio"*. `.pz-studio--hub` computed an opaque `rgb(207,196,176)` over the whole viewport. Both grounds are cleared, the base rule's and the light one's — and note the light one ramped `#f3f6f9 → #dfe8f0`, the retired **Cool Silver** palette this file already records as *"wrong palette, looks so different"*, laying cold grey over warm art. ⚠ The override is `html[data-home-theme] .pz-studio.pz-studio` — (0,3,1) deliberately, because matching the (0,2,1) of `html[data-home-theme='light'] .pz-studio` would leave it to file order against a stylesheet that may load either side. Same coin toss the Wellbeing landing lost. ⚠ The `--pz-*` custom properties are untouched: clearing a ground must not restyle the cards and given cells that read them.

⚠ **VOID RUNNER IS THE ONE PLACE A GAMEPLAY BACKGROUND IS WRONG RATHER THAN MISSING** — *"two circles that appears on the screen up for no reason and there is blurring in the screen"*. `.vr-root` had been given the `::before` art layer on the reasoning, written into a comment and never tested, that *"a canvas cannot be seen through"*. Void Runner's canvas is not opaque, so the art rendered **through** the 3D world. The layer and its `> * { z-index: 1 }` partner are both gone — leaving the latter would impose a stacking context on every child of a hand-tuned 3D game for nothing.

⚠ **`.vr-root` ALSO HAD NO OPAQUE GROUND OF ITS OWN, AND THE SHORTHAND HID IT.** `--play-surface-deep` is an **image**, so `background: …gradients…, var(--play-surface-deep)` left `background-color` computing as `rgba(0,0,0,0)`. That was harmless while an ancestor painted a ground and stopped being harmless the moment the Studio's ground went transparent. Split into `background-color` + `background-image`.

⚠ **THE TWO CIRCLES WERE IN THE SCENE, NOT THE BACKGROUND — and the obvious theory was wrong.** They look exactly like the app background's watercolour planets, so bleed-through is where everyone starts. It is not: the canvas is `alpha: false`, covers the full viewport, `elementFromPoint` returns CANVAS at both discs, and forcing everything behind it magenta left them untouched. They are `warmWorld` and `coolMoon` — `depthWrite: false, fog: false` and transparent, so they draw after the opaque pass and float *through* the corridor walls as flat discs in mid-air. Hidden with `visible = false` rather than deleted, because the run loop still animates `horizonGroup`.

⚠ **THE PALE BAND IN THE LOWER SCREEN WAS A SEPARATE BUG, AND `scene.background` IS WHY.** It was `VR.skyLow: '#5d4a4f'`, the scene's own *"restrained warm horizon"*, sitting at **stop 1.0** of `makeSky()`'s gradient. `scene.background` holding a plain texture is drawn **screen aligned and stretched to the viewport** — it does not move with the camera — so the last stop is painted permanently across the bottom edge of the screen. A horizon colour was doing a floor's job, and the tunnel's near end always sat on a broad pale block with a hard horizontal seam.

⚠ **IT IS WORST IN PORTRAIT, WHICH IS WHY IT WAS REPORTED FROM A PHONE.** The gradient is in *screen* space, so a 9:16 viewport gives the bottom stop far more pixels than 16:9 — and puts them directly under the ship. `skyLow` now peaks at **0.54** and the gradient is **fully resolved to `VR.floor` by 0.70**, held there to 1.0. ⚠ **The stop had to clear the FLOOR'S NEAR EDGE, and that edge MOVES** — measured across frames it lands anywhere from ~55% to ~99% of screen height, because camera pitch changes with speed. A first attempt peaked `skyLow` at 0.72 and still washed the bottom on the frames where the floor ended high. The band now sits behind the tunnel, around the vanishing point, where it reads as an intended dusk horizon in both aspect ratios. ⚠ The floor stops are **derived** from `VR.floor`, not written out again — a second literal of the same colour is the half-migrated pair this file keeps recording. **Check this one in portrait; a desktop window hides most of it.**

⚠ **FOUR PROBES LIED BEFORE THIS WAS SETTLED, and every one of them looked authoritative.** (1) `elementFromPoint` reported CANVAS everywhere while silently ignoring six full-screen overlays that set `pointer-events: none` — it cannot see a layer that paints but takes no input, so **enumerate children and their rects instead**. (2) A `drawImage`-based pixel sampler returned **rgb(0,0,0) at every height** — not a black screen but an empty backbuffer, since a WebGL canvas without `preserveDrawingBuffer` reads back cleared once the frame is presented. (3) A "walk up from the bottom to the first non-warm row" detector reported **no warm band at all** on a screen that obviously had one, because the fix had just made the bottom row floor-coloured — it measured a transition where the question was a **span**. (4) The corrected span detector then sampled a column that runs straight through the **amber tunnel arch**, and duly reported warm pixels that were architecture, not sky.

**SwiftShader was the tempting explanation throughout and was wrong** — the band was confirmed present on the real GPU too. Sample the **screenshot**, never the live WebGL canvas; and in a scene this busy, prefer looking at the picture over trusting a colour heuristic that cannot tell a sunset from a girder.

⚠ **KAWNERA APPEARED TWICE ON ONE SCREEN**, once in the eyebrow and once as `<b>KAWNERA</b>` in the footer. Both dropped, **in EN and AR** — the AR half sits ~25 lines below the English one and is this repo's most-repeated string bug. ⚠ The lazy-load fallback still shows the wordmark once while the shelf mounts; that is a different screen and shows it once, so it was left. Note it made the first verification pass *look* like a failure: the sweep's fixed 13s wait captured the spinner, and software rendering is slower than the wait was tuned for. **Wait on the element, not the clock.**

⚠ **THE FLOATING Dr. KAWKAB GUIDE COVERED VOLUME ART ON A PHONE** and is removed. Its strings are deliberately left in the dictionary. ⚠ If it ever returns, it must **not** come back as a 3D rig: that is a second WebGL context on a tab whose sibling (Home's universe) already holds one, and the browser evicting the oldest is what used to blank the universe. The 2D `kawkab-planet.webp` sprite is the safe form.

⚠ **THE LEARN FOOTER IS BLACK IN BOTH APPEARANCES, SO ITS INK CANNOT COME FROM `--universe-ink`.** That token flips, and in light theme it is near-black — invisible on a black bar. `--fx-kw-foot-bg` / `-ink` / `-accent` are fixed literals on purpose, in `tokens.css` where the `--fx-` prefix exempts them from `audit:design`'s raw-colour rule. That prefix is exactly what it is for; do not put them in the feature file. The ink and accent are the app's existing parchment and gold, so a permanently dark strip still reads as this product.

⚠ **THE HOME PLANET SHRANK WITHOUT DRAGGING THE SYSTEM IN WITH IT** — *"make the 3d planet in home smaller"*. `CENTER_RADIUS` was one lever for six things: the particle sphere, the solid body, the halo, the glow sprite, the tap sphere **and the three orbit radii the user planets ride on**. Scaling it would have pulled the user planets inward and crowded the middle, which is not what was asked. The body group derives from a new `CENTER_BODY` (0.74×) and the orbits stay on `CENTER_RADIUS`. ⚠ **`hitSphere` MUST scale with the body** — it is invisible, so a body that shrank while its hit target did not leaves a ring of dead space that still swallows taps aimed past the planet. Nobody can see that fault and everybody feels it.

**Verified on screen**, 2 viewports × 2 themes (390×844 and 1366×577, light and dark), zero exceptions: correct art variant on body and in-game, Studio ground transparent, Void Runner root opaque with no stray discs, Learn footer `rgb(6,8,14)` on `rgb(240,226,192)` with zero KAWNERA occurrences, and the Detective stage scrolling its full 295px **under a real dispatched wheel event** — not merely reporting a `scrollHeight`, which says the content is tall and nothing about whether a human can reach the bottom.
