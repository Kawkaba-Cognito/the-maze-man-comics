...
**Still open** (raised in the audit, not built): a validated outcome measure (WHO-5 is the obvious fit — 5 items, free, Arabic versions exist); CBT-I for the Sleep programme, which currently offers only the weakest arms; a practice for Relationships, which is at present one quiz *about yourself*; `FORMATION_DAYS = 66` presented as a target when Lally et al. (2010) reported it as the **median** of an 18–254 day range — the SRBAI would let the app measure automaticity instead of assuming it; and the uncredited Clear/Fogg framings in Habits, next to the one genuinely evidence-based mechanic there (if-then anchors = implementation intentions, Gollwitzer & Sheeran 2006) which is never named.
