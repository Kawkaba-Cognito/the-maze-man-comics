...
- **Domains & live games** (per domain.config.js): attention `cancel-task, mot, train-switch` · speed `speed-match, math-gates, intercept` · memory `story-grid, keep-track, paired-associates` · language `wordle, synonyms, trivia` · reasoning `rush-hour, raven-matrices, detective` · flexibility `mirror-world, task-switch, sort-shift`.
