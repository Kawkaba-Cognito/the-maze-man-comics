...
**Deploying is automatic (2026-07-16)**: every push to `main` on `origin` (Kawkaba-Cognito) triggers `.github/workflows/deploy.yml`, which installs, runs **`audit:sec` → `validate:wordmaze` → `audit:pacing` → `validate:storyq` → `validate:liars` → `validate:gatekeeper` → `audit:fq` → `validate:rh` → `audit:mot` → `audit:gamekeys` → `audit:design`** (all eleven block the deploy), builds, and publishes `dist/` to the `gh-pages` branch — the branch GitHub Pages serves. It can also be run by hand from the repo's Actions tab (workflow_dispatch). The `cognitive` mirror does **not** auto-deploy; push `gh-pages` there manually if the mirror should stay current.
...
- **`registry.js`** — the spine: imports every `domains/<domain>/domain.config.js`. `lazyGames.js` auto-builds the lazy component map from it.
  ...
  ⚠️ **Adding a game is EIGHT wiring spots, not one** (was "seven" 2026-08-10; corrected again 2026-08-20 when The Gate needed one the list did not name). Wiring Keep Track and then The Gate touched: `domain.config.js` · `rating.js` (`RATED_GAMES` **and** `ACTIVE_RATED_GAME_KEYS`) · `shared/gameScience.js` · `shared/tutorials/trainingMeta.js` · `shared/GamePlanetTile.jsx` (**two** places: `COVER_KEYS` and `COVER_FILE_OVERRIDES`) · **`personalization/trainingRecommendations.js` (`ACTIVE_GAME_KEYS`)** · `package.json` (if it has a content bank needing a `validate:*` gate) · `lazyGames.js` (**only** if the assessment battery runs it).
...
**GIVE THE SCORED DECISION THE HEAVIER GESTURE** (2026-08-20). Detective's holding cell — the one thing in the game that is judged apart from the answer — was reached by tapping a suspect card a third time, at the end of the private notebook cycle (unmarked → cleared → held → **in the cell**). So the only committing act in the game was also the easiest thing on the board to trigger by accident, while taking notes. It is now: **tap** cycles the private marks, **drag a suspect down into the cell** locks them up. A mark is a thought and costs a tap; an arrest is a commitment and costs a deliberate gesture across the screen. Weight the input to match what the act costs.

⚠ **The drag moves the CARD, not a floating clone.** `#ui-shell` is a transformed ancestor, so `position: fixed` is trapped inside it (the same trap the universe modals hit — they are portalled to body for this reason). A `transform` is relative to the element's own layout box and cannot be trapped, and the cell sits directly under the line-up so the whole gesture is ~80px. ⚠ **And the cell is ALSO a tap-through picker** (nobody → each suspect → nobody), which is not a convenience: a drag needs a pointer that can press, move and hold, and a keyboard has none. Removing it to make dragging the only way in re-creates the Mirror World lockout recorded below — an accessible control that renders, registers taps, and cannot reach the state.
