...
⚠ **THE CHARACTER WAS A DIFFERENT CHARACTER, IN TWO PLACES.** `quizShared` rendered `CosmosCharacter faceOnly` — `kawkab-face.png`, the retired sprite — as a cropped head peeking over a bubble reading *"Hi, I'm Kawkab!"*, while the Training hub's Kawkab is `kawkab-planet.webp`. `KawkabSprite`'s own header already documented this ("CosmosCharacter — 2D, but a different character"); Wellbeing was never included in that sweep. And `habitReminders.js` shipped **`guide-fox-sprite.webp`** — a fox — as the notification icon, which is the one place the mascot appears *outside* the app. Both are the shared `KawkabSprite` now, and he stands at the foot of the landing as the guide.

⚠ **HE IS AT THE FOOT OF THE SKY, NOT THE CENTRE OF IT.** Training is a hub-and-spoke wheel with the mascot at the nexus; Wellbeing's five areas are independent and nothing connects them (see the `CAT_LAYOUT` note). Centring him would import Training's geometry and collapse the one deliberate distinction between the two landings. He also **speaks the hint** rather than standing beside it — there was already an orphan instruction line down there belonging to nobody.

⚠ **THE PER-AREA HUE WAS DEFINED, WIRED AND INVISIBLE.** Each category has carried a `color` since the screen was written, painted as `${color}42` over `${color}20` — 26% then 12% alpha — behind a **Fluent emoji**. All five rendered as the same grey-lilac disc; only the caption told Sleep from Relationships. `wellbeing.css` now gives each area a `core`/`lit` pair that flips with the theme, and `.rx-body` shades a real sphere with the old icon riding on it as an emblem at 46%. ⚠ Note the underlying inequality that made this necessary: `planetIcons.js` gives the six **training domains** hand-painted `category-drawings-2026` art and gives the five **wellbeing pillars** emoji. Five paintings is the right long-term fix and is the owner's call.
...
⚠ **ON A PHONE THE GUIDE'S ROW REVERSES.** Centred, the sprite landed at ~20% of a 390px stage — exactly where `CAT_LAYOUT` puts Personality (x: 22) — and sat on that planet's caption. `justify-content: flex-end` was the obvious fix and moved him about ten pixels, because sprite + gap + bubble is ~333px inside a 358px stage. `row-reverse` costs no width and moves him the full width of the bubble.
