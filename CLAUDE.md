...
**Deploying is automatic (2026-07-16)**: every push to `main` on `origin` (Kawkaba-Cognito) triggers `.github/workflows/deploy.yml`, which installs, runs **`audit:sec` → `validate:wordmaze` → `audit:pacing` → `audit:fq` → `validate:rh` → `audit:mot` → `audit:design`** (all seven block the deploy), builds, and publishes `dist/` to the `gh-pages` branch — the branch GitHub Pages serves. It can also be run by hand from the repo's Actions tab (workflow_dispatch). The `cognitive` mirror does **not** auto-deploy; push `gh-pages` there manually if the mirror should stay current.
...
- **Domains & live games** (per domain.config.js): attention `cancel-task, mot, train-switch` · speed `speed-match, math-gates, intercept` (Survival is **Rift Defense** — sectors, waves, a shield and between-sector upgrades, added 2026-08-14; Levels/Pass n Play stay the plain timing task) · memory `story-grid, keep-track, paired-associates` · language `wordle, synonyms, trivia` · reasoning `rush-hour, raven-matrices, detective` · flexibility `mirror-world, task-switch, sort-shift`.
