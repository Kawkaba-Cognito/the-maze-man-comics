...
⚠ **`--game-bg` IS DERIVED, NOT RE-DECLARED.** Four parallel declarations (light/dark × landscape/portrait) is exactly the shape that ships a half-migrated pair — one retargeted, its partner left as a stale literal — which this file records happening five times in one day. Deriving makes the disagreement inexpressible. The `gameplay-backgrounds-v2/` files are kept, unreferenced, as the hook.
