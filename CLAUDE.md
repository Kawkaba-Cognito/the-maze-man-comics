...
⚠ **IT IS WORST IN PORTRAIT, WHICH IS WHY IT WAS REPORTED FROM A PHONE.** The gradient is in *screen* space, so a 9:16 viewport gives the bottom stop far more pixels than 16:9 — and puts them directly under the ship. `skyLow` now peaks at **0.72** and the gradient falls to `VR.floor` by 1.0, so the dusk horizon still reads at a height where a horizon belongs and the bottom of the frame matches the surface being flown over. ⚠ The final stop is **derived** from `VR.floor`, not written out again — a second literal of the same colour is the half-migrated pair this file keeps recording. **Check this one in portrait; a desktop window hides most of it.**

⚠ **THREE PROBES LIED BEFORE THIS WAS SETTLED, and each looked authoritative.** `elementFromPoint` reported CANVAS everywhere while silently ignoring six full-screen overlays that set `pointer-events: none` — it cannot see a layer that paints but takes no input, so **enumerate children and their rects instead**. A `drawImage`-based pixel sampler returned **rgb(0,0,0) at every height**, which is not a black screen but an empty backbuffer: a WebGL canvas without `preserveDrawingBuffer` reads back cleared once the frame is presented. And SwiftShader was the tempting explanation for the whole thing — the band was confirmed present on the **real GPU** too. Sample the **screenshot**, never the live WebGL canvas, and look at it.
