...
⚠ **THE PALE BAND IN THE LOWER SCREEN IS NOT A BUG AND MUST NOT BE "FIXED" BLIND** (checked 2026-09-08, GPU **and** SwiftShader, both themes). It is `VR.skyLow: '#5d4a4f'` — the scene's own *"restrained warm horizon"* in `makeSky()` — showing where the tunnel geometry ends. It is larger in dark than light because the sky is a `CanvasTexture` gradient behind geometry, not because anything leaks in. ⚠ **Two probes lied about this before it was settled**: `elementFromPoint` reported CANVAS everywhere while ignoring six full-screen overlays that set `pointer-events: none` — it cannot see a layer that paints but does not take input, so **enumerate children and their rects instead**. If the owner ever asks for that band to go, it is a `skyLow` colour decision, not a compositing repair.
