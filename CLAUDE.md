...
A wave is a rung now. `fqLadderWaveTable` builds `[level][wave] = { tc, time }` once: targets climb `0.85 → 1.15×` the level's count while the clock falls `1.16 → 0.86×` its average, geometrically. Both levers are ones `expertTargetSecForSetSize` already prices, so feasibility stays true per board by construction. Measured at L60: **13 targets in 23s → 16 in 20s**, i.e. 1.77 s/target down to 1.25.
