import{a as e,n as t,t as n}from"./jsx-runtime-DIAExeAg.js";import{B as r,Ct as i,Dt as a,Kt as o,Ot as s,Pt as c,V as l,_ as u,d,f,g as ee,gt as te,ht as ne,nt as re,qt as p,t as ie,tt as ae,vt as m}from"./three.module-cCYEhw8-.js";import{r as oe}from"./c3dViewport-D3K1ZAxX.js";var h=e(t()),g=n(),_={css:`linear-gradient(180deg,
    ...
    #8a6553 72%, #b8845d 86%, #ddaf80 100%)`,bodyTop:[.048,.052,.078],bodyBot:[.108,.072,.066],rim:[1,.6,.3],halo:[1,.7,.42],star:[.96,.94,.88],bgDim:.5},v=1.35,se=.36,y=.8,ce=3.2,le=3.2,b=6,ue=3.6;function de(e,t){let n=new Float32Array(e*3),r=new Float32Array(e),i=new Float32Array(e);for(let a=0;a<e;a++){let e=Math.random()*2-1,o=Math.random()*Math.PI*2,s=Math.sqrt(1-e*e),c=t*(.9+.1*Math.random());n[a*3]=s*Math.cos(o)*c,n[a*3+1]=e*c,n[a*3+2]=s*Math.sin(o)*c,r[a]=Math.random(),i[a]=.6+Math.random()}let a=new f;return a.setAttribute(`position`,new d(n,3)),a.setAttribute(`aRand`,new d(r,1)),a.setAttribute(`aSize`,new d(i,1)),a}function fe(e,t){let n=new Float32Array(e*3),r=new Float32Array(e),i=new Float32Array(e);for(let a=0;a<e;a++){let o=a/e*Math.PI*2+Math.random()*.04,s=(Math.random()-.5)*.22,c=t*(.96+Math.random()*.1);n[a*3]=Math.cos(o)*c,n[a*3+1]=Math.sin(o)*c*.42+s*c,n[a*3+2]=Math.sin(o)*c*.18,r[a]=Math.random(),i[a]=.7+Math.random()*1.2}let a=new f;return a.setAttribute(`position`,new d(n,3)),a.setAttribute(`aRand`,new d(r,1)),a.setAttribute(`aSize`,new d(i,1)),a}function pe(e){let t=0;for(let n=0;n<e.length;n++)t=t*31+e.charCodeAt(n)|0;return Math.abs(t)%1e3/1e3}var x=(0,h.forwardRef)(function({planets:e},t){let n=(0,h.useRef)(null),[x,me]=(0,h.useState)(0),he=(0,h.useRef)(!1),[S,C]=(0,h.useState)(!1);(0,h.useEffect)(()=>{let e=()=>{let e=document.documentElement.dataset.homeTheme===`light`;he.current=e,C(e)};e();let t=new MutationObserver(e);return t.observe(document.documentElement,{attributes:!0,attributeFilter:[`data-home-theme`]}),()=>t.disconnect()},[]);let w=(0,h.useRef)({syncPlanets:()=>{},dissolvePlanet:()=>{},reformPlanet:()=>{},pulseCenter:()=>{},setRunning:()=>{}});return(0,h.useImperativeHandle)(t,()=>({dissolvePlanet:e=>w.current.dissolvePlanet(e),reformPlanet:e=>w.current.reformPlanet(e),pulseCenter:()=>w.current.pulseCenter(),setRunning:e=>w.current.setRunning(e)}),[]),(0,h.useEffect)(()=>{let e=n.current;if(!e)return;let t=window.matchMedia(`(prefers-reduced-motion: reduce)`).matches,h=window.matchMedia(`(pointer: fine)`).matches,g=h?1:1.45,x=new a,S=new ne(55,1,.1,200);S.position.set(0,0,7);let C=new ie({antialias:!0,alpha:!0}),ge=e=>{e.preventDefault(),cancelAnimationFrame(Z),Z=0},_e=()=>{me(e=>e+1)};C.domElement.addEventListener(`webglcontextlost`,ge,!1),C.domElement.addEventListener(`webglcontextrestored`,_e,!1),C.setPixelRatio(Math.min(window.devicePixelRatio||1,h?1.5:1.35)),C.setClearColor(0,0),C.domElement.style.display=`block`,e.appendChild(C.domElement);let ve=de(h?22e3:16e3,v),ye=[],be=[];for(let e=0;e<b;e++)ye.push(new p),be.push(-99);let T=new s({transparent:!0,depthWrite:!1,blending:2,uniforms:{uNow:{value:0},uDim:{value:0},uBoost:{value:g},uBreath:{value:1},uLight:{value:0},uBodyTop:{value:new p(..._.bodyTop)},uBodyBot:{value:new p(..._.bodyBot)},uRim:{value:new p(..._.rim)},uTouches:{value:ye},uStarts:{value:be}},vertexShader:`
      ...
      `}),E=new m(ve,T);x.add(E);let xe=new s({uniforms:{uNow:{value:0},uTop:{value:new p(..._.bodyTop)},uBot:{value:new p(..._.bodyBot)},uRim:{value:new p(..._.rim)}},vertexShader:`
        varying vec3 vNv;
        varying vec3 vObj;
        varying vec3 vView;
        void main() {
      ...
      `,transparent:!0,depthWrite:!1}),D=new ae(new c(v*.98,64,48),xe);D.visible=!1,D.renderOrder=-1,x.add(D);let Se=fe(h?220:480,v*1.38),O=new s({transparent:!0,depthWrite:!1,blending:2,uniforms:{uNow:{value:0},uDim:{value:0},uBoost:{value:g},uBreath:{value:1},uLight:{value:0},uWarm:{value:new p(..._.halo)}},vertexShader:`
        attribute float aRand;
        attribute float aSize;
        uniform float uNow;
        uniform float uBoost;
      ...
      `}),k=new m(Se,O);x.add(k);let Ce=new te(v*(h?3.6:5.2),v*(h?3.6:5.2)),A=new s({transparent:!0,depthWrite:!1,blending:2,uniforms:{uNow:{value:0},uDim:{value:0},uBoost:{value:g},uBreath:{value:1},uLight:{value:0}},vertexShader:`
        varying vec2 vUv;
        void main() {
          vUv = uv;
      ...
      `}),we=new ae(Ce,A);x.add(we);let j=[{r:v*1.55,speed:.5,tiltX:.55,tiltZ:.3,col:[1,.85,.58]},{r:v*1.85,speed:-.36,tiltX:-.72,tiltZ:.18,col:[.62,.85,1]},{r:v*2.15,speed:.27,tiltX:.24,tiltZ:-.6,col:[.85,.7,1]}],M=h?48:60,N=j.length*M,Te=new Float32Array(N*3),Ee=new Float32Array(N),P=new Float32Array(N*4),De=new Float32Array(N*2),F=new Float32Array(N*3);for(let e=0;e<j.length;e++){let t=j[e],n=e/j.length*Math.PI*2;for(let r=0;r<M;r++){let i=e*M+r;Ee[i]=r/(M-1),P[i*4]=t.r,P[i*4+1]=t.speed,P[i*4+2]=n,P[i*4+3]=n*2.7,De[i*2]=t.tiltX,De[i*2+1]=t.tiltZ,F[i*3]=t.col[0],F[i*3+1]=t.col[1],F[i*3+2]=t.col[2]}}let I=new f;I.setAttribute(`position`,new d(Te,3)),I.setAttribute(`aT`,new d(Ee,1)),I.setAttribute(`aOrb`,new d(P,4)),I.setAttribute(`aTilt`,new d(De,2)),I.setAttribute(`aCol`,new d(F,3));let L=new s({transparent:!0,depthWrite:!1,blending:2,uniforms:{uNow:{value:0},uDim:{value:0},uBoost:{value:g}},vertexShader:`
        attribute float aT;
        attribute vec4 aOrb;
      ...
      `}),Oe=new m(I,L);Oe.frustumCulled=!1,Oe.visible=!t,x.add(Oe);let R=new ae(new c(v*1.12,16,16),new re({visible:!1}));x.add(R);let z=h?700:900,B=new Float32Array(z*3),ke=new Float32Array(z),Ae=new Float32Array(z),je=new Float32Array(z),Me=new Float32Array(z);for(let e=0;e<z;e++){let t=Math.random()*2-1,n=Math.random()*Math.PI*2,r=Math.sqrt(1-t*t),i=25+Math.random()*45;B[e*3]=r*Math.cos(n)*i,B[e*3+1]=t*i,B[e*3+2]=r*Math.sin(n)*i,ke[e]=Math.random()*Math.PI*2,Ae[e]=.2+Math.random()*1.8,je[e]=Math.random()<.35?1:.25+Math.random()*.4,Me[e]=.5+Math.random()*1.3}let V=new f;V.setAttribute(`position`,new d(B,3)),V.setAttribute(`aPhase`,new d(ke,1)),V.setAttribute(`aSpeed`,new d(Ae,1)),V.setAttribute(`aDepth`,new d(je,1)),V.setAttribute(`aSize`,new d(Me,1));let H=new s({transparent:!0,depthWrite:!1,blending:2,uniforms:{uNow:{value:0},uDim:{value:0},uCol:{value:new p(.85,.82,.72)}},vertexShader:`
      ...
      `});x.add(new m(V,H));let U=h?0:350,W=new Float32Array(U*3),Ne=new Float32Array(U),Pe=new Float32Array(U);for(let e=0;e<U;e++)W[e*3]=(Math.random()-.5)*28,W[e*3+1]=(Math.random()-.5)*22,W[e*3+2]=(Math.random()-.5)*18-2,Ne[e]=Math.random(),Pe[e]=1.2+Math.random()*2.8;let G=new f;G.setAttribute(`position`,new d(W,3)),G.setAttribute(`aRand`,new d(Ne,1)),G.setAttribute(`aSize`,new d(Pe,1));let Fe=new s({transparent:!0,depthWrite:!1,blending:2,uniforms:{uNow:{value:0},uDim:{value:0}},vertexShader:`
        attribute float aRand;
        attribute float aSize;
        uniform float uNow;
        varying float vAlpha;
