import{a as e,n as t,t as n}from"./jsx-runtime-DIAExeAg.js";import{At as r,Et as i,H as a,Qt as o,V as s,Zt as c,_ as l,f as u,it as d,jt as f,p,rt as ee,t as te,v as ne,vt as re,xt as m,yt as ie,zt as ae}from"./three.module-C36HyJKG.js";import{r as oe}from"./c3dViewport-D3K1ZAxX.js";var h=e(t()),g=n(),_=1.35,se=.36,v=.8,ce=3.2,le=3.2,y=6,ue=3.6;function de(e,t){let n=new Float32Array(e*3),r=new Float32Array(e),i=new Float32Array(e);for(let a=0;a<e;a++){let e=Math.random()*2-1,o=Math.random()*Math.PI*2,s=Math.sqrt(1-e*e),c=t*(.9+.1*Math.random());n[a*3]=s*Math.cos(o)*c,n[a*3+1]=e*c,n[a*3+2]=s*Math.sin(o)*c,r[a]=Math.random(),i[a]=.6+Math.random()}let a=new p;return a.setAttribute(`position`,new u(n,3)),a.setAttribute(`aRand`,new u(r,1)),a.setAttribute(`aSize`,new u(i,1)),a}function fe(e,t){let n=new Float32Array(e*3),r=new Float32Array(e),i=new Float32Array(e);for(let a=0;a<e;a++){let o=a/e*Math.PI*2+Math.random()*.04,s=(Math.random()-.5)*.22,c=t*(.96+Math.random()*.1);n[a*3]=Math.cos(o)*c,n[a*3+1]=Math.sin(o)*c*.42+s*c,n[a*3+2]=Math.sin(o)*c*.18,r[a]=Math.random(),i[a]=.7+Math.random()*1.2}let a=new p;return a.setAttribute(`position`,new u(n,3)),a.setAttribute(`aRand`,new u(r,1)),a.setAttribute(`aSize`,new u(i,1)),a}function pe(e){let t=0;for(let n=0;n<e.length;n++)t=t*31+e.charCodeAt(n)|0;return Math.abs(t)%1e3/1e3}var b=(0,h.forwardRef)(function({planets:e},t){let n=(0,h.useRef)(null),[b,me]=(0,h.useState)(0),x=(0,h.useRef)({syncPlanets:()=>{},dissolvePlanet:()=>{},reformPlanet:()=>{},pulseCenter:()=>{},setRunning:()=>{}});return(0,h.useImperativeHandle)(t,()=>({dissolvePlanet:e=>x.current.dissolvePlanet(e),reformPlanet:e=>x.current.reformPlanet(e),pulseCenter:()=>x.current.pulseCenter(),setRunning:e=>x.current.setRunning(e)}),[]),(0,h.useEffect)(()=>{let e=n.current;if(!e)return;let t=window.matchMedia(`(prefers-reduced-motion: reduce)`).matches,h=window.matchMedia(`(pointer: fine)`).matches,g=h?1:1.45,b=new r,S=new re(55,1,.1,200);S.position.set(0,0,7);let C=new te({antialias:!0}),he=e=>{e.preventDefault(),cancelAnimationFrame(Z),Z=0},ge=()=>{me(e=>e+1)};C.domElement.addEventListener(`webglcontextlost`,he,!1),C.domElement.addEventListener(`webglcontextrestored`,ge,!1),C.setPixelRatio(Math.min(window.devicePixelRatio||1,h?1.5:1.35)),C.setClearColor(0,1),C.domElement.style.display=`block`,e.appendChild(C.domElement);let _e=de(h?22e3:16e3,_),ve=[],ye=[];for(let e=0;e<y;e++)ve.push(new o),ye.push(-99);let w=new f({transparent:!0,depthWrite:!1,blending:2,uniforms:{uNow:{value:0},uDim:{value:0},uBoost:{value:g},uBreath:{value:1},uTouches:{value:ve},uStarts:{value:ye}},vertexShader:`
      ...
      `}),T=new m(_e,w);b.add(T);let be=fe(h?220:480,_*1.38),E=new f({transparent:!0,depthWrite:!1,blending:2,uniforms:{uNow:{value:0},uDim:{value:0},uBoost:{value:g},uBreath:{value:1}},vertexShader:`
        attribute float aRand;
        attribute float aSize;
        uniform float uNow;
        uniform float uBoost;
      ...
      `}),D=new m(be,E);b.add(D);let xe=new ie(_*(h?3.6:5.2),_*(h?3.6:5.2)),O=new f({transparent:!0,depthWrite:!1,blending:2,uniforms:{uNow:{value:0},uDim:{value:0},uBoost:{value:g},uBreath:{value:1}},vertexShader:`
        varying vec2 vUv;
        void main() {
          vUv = uv;
      ...
      `}),Se=new ee(xe,O);b.add(Se);let k=[{r:_*1.55,speed:.5,tiltX:.55,tiltZ:.3,col:[1,.85,.58]},{r:_*1.85,speed:-.36,tiltX:-.72,tiltZ:.18,col:[.62,.85,1]},{r:_*2.15,speed:.27,tiltX:.24,tiltZ:-.6,col:[.85,.7,1]}],A=h?48:60,j=k.length*A,Ce=new Float32Array(j*3),we=new Float32Array(j),M=new Float32Array(j*4),Te=new Float32Array(j*2),N=new Float32Array(j*3);for(let e=0;e<k.length;e++){let t=k[e],n=e/k.length*Math.PI*2;for(let r=0;r<A;r++){let i=e*A+r;we[i]=r/(A-1),M[i*4]=t.r,M[i*4+1]=t.speed,M[i*4+2]=n,M[i*4+3]=n*2.7,Te[i*2]=t.tiltX,Te[i*2+1]=t.tiltZ,N[i*3]=t.col[0],N[i*3+1]=t.col[1],N[i*3+2]=t.col[2]}}let P=new p;P.setAttribute(`position`,new u(Ce,3)),P.setAttribute(`aT`,new u(we,1)),P.setAttribute(`aOrb`,new u(M,4)),P.setAttribute(`aTilt`,new u(Te,2)),P.setAttribute(`aCol`,new u(N,3));let F=new f({transparent:!0,depthWrite:!1,blending:2,uniforms:{uNow:{value:0},uDim:{value:0},uBoost:{value:g}},vertexShader:`
        attribute float aT;
        attribute vec4 aOrb;
      ...
      `}),Ee=new m(P,F);Ee.frustumCulled=!1,Ee.visible=!t,b.add(Ee);let I=new ee(new ae(_*1.12,16,16),new d({visible:!1}));b.add(I);let L=h?700:900,R=new Float32Array(L*3),De=new Float32Array(L),Oe=new Float32Array(L),ke=new Float32Array(L),Ae=new Float32Array(L);for(let e=0;e<L;e++){let t=Math.random()*2-1,n=Math.random()*Math.PI*2,r=Math.sqrt(1-t*t),i=25+Math.random()*45;R[e*3]=r*Math.cos(n)*i,R[e*3+1]=t*i,R[e*3+2]=r*Math.sin(n)*i,De[e]=Math.random()*Math.PI*2,Oe[e]=.2+Math.random()*1.8,ke[e]=Math.random()<.35?1:.25+Math.random()*.4,Ae[e]=.5+Math.random()*1.3}let z=new p;z.setAttribute(`position`,new u(R,3)),z.setAttribute(`aPhase`,new u(De,1)),z.setAttribute(`aSpeed`,new u(Oe,1)),z.setAttribute(`aDepth`,new u(ke,1)),z.setAttribute(`aSize`,new u(Ae,1));let B=new f({transparent:!0,depthWrite:!1,blending:2,uniforms:{uNow:{value:0},uDim:{value:0}},vertexShader:`
      ...
      `});b.add(new m(z,B));let V=h?0:350,H=new Float32Array(V*3),je=new Float32Array(V),Me=new Float32Array(V);for(let e=0;e<V;e++)H[e*3]=(Math.random()-.5)*28,H[e*3+1]=(Math.random()-.5)*22,H[e*3+2]=(Math.random()-.5)*18-2,je[e]=Math.random(),Me[e]=1.2+Math.random()*2.8;let U=new p;U.setAttribute(`position`,new u(H,3)),U.setAttribute(`aRand`,new u(je,1)),U.setAttribute(`aSize`,new u(Me,1));let W=new f({transparent:!0,depthWrite:!1,blending:2,uniforms:{uNow:{value:0},uDim:{value:0}},vertexShader:`
        attribute float aRand;
        attribute float aSize;
        uniform float uNow;
        varying float vAlpha;
    ...
    `}),r=new m(q,n);return b.add(r),{points:r,mat:n,phase:t}}function Re(t){let n=e.clientWidth||1,r=e.clientHeight||1,i=2*(S.position.z-ce)*Math.tan(S.fov*Math.PI/360)/r,a=Math.min(.9*n,560)*i,s=Math.min(.68*r,620)*i,c=q.getAttribute(`aPaper`),l=t.points.position,u=t.frozenRot,d=new o,f=new o(0,1,0);for(let e=0;e<Ie;e++){let t,n;if(e%3==0){let e=Math.random()*2*(a+s);e<a?(t=e-a/2,n=-s/2):e<a+s?(t=a/2,n=e-a-s/2):e<2*a+s?(t=e-a-s-a/2,n=s/2):(t=-a/2,n=e-2*a-s-s/2)}else t=(Math.random()-.5)*a,n=(Math.random()-.5)*s;d.set(t,n,ce+(Math.random()-.5)*.06).sub(l).applyAxisAngle(f,-u),c.setXYZ(e,d.x,d.y,d.z)}c.needsUpdate=!0}function ze(){let e=2*(S.position.z-v)*Math.tan(S.fov*Math.PI/360);return{w:e*S.aspect,h:e}}function Be(e,t){let{w:n,h:r}=ze();return{x:(e/100-.5)*n,y:(.5-t/100)*r}}let Ve=-99,He=new i,Ue=new c,Y=0,X=0,We=new l;function Ge(e){ve[X].copy(e),ye[X]=We.getElapsedTime(),X=(X+1)%y}x.current={syncPlanets(e){let t=new Set;for(let n of e){t.add(n.id);let e=J.get(n.id);if(!e){let t=pe(String(n.id));e={...Le(n.color,t),dissolve:0,target:0,morph:0,morphTarget:0,frozenRot:0,removing:!1,phase:t},e.dissolve=1,e.mat.uniforms.uDissolve.value=1,J.set(n.id,e)}e.xPct=n.x,e.yPct=n.y,e.mat.uniforms.uColor.value.set(n.color)}for(let[e,n]of J)!t.has(e)&&!n.removing&&(n.removing=!0,n.target=1)},dissolvePlanet(e){let t=J.get(e);t&&(t.frozenRot=t.points.rotation.y,Re(t),t.morphTarget=1)},reformPlanet(e){let t=J.get(e);t&&(t.morphTarget=0)},pulseCenter(){let e=We.getElapsedTime();if(e-Ve<1.4)return;Ve=e;let t=new o(Math.random()-.5,Math.random()-.5,.6+Math.random()).normalize().multiplyScalar(_);Ge(T.worldToLocal(t))}};function Ke(e){let t=C.domElement.getBoundingClientRect();Ue.set((e.clientX-t.left)/t.width*2-1,-((e.clientY-t.top)/t.height)*2+1),He.setFromCamera(Ue,S);let n=He.intersectObject(I)[0];n&&Ge(T.worldToLocal(n.point.clone()).normalize().multiplyScalar(_))}C.domElement.addEventListener(`pointerdown`,Ke);function qe(){let t=e.clientWidth||1,n=e.clientHeight||1;S.aspect=t/n,S.updateProjectionMatrix(),C.setSize(t,n)}qe();let Je=new ResizeObserver(qe);Je.observe(e);let Z=0,Q=!0,Ye=0,$=0;function Xe(){if(!Q)return;Z=requestAnimationFrame(Xe),Y=We.getElapsedTime();let e=Math.min(Y-Ye,.05);Ye=Y;let n=t?0:Y,r=t?1:1+.045*Math.sin(Y*Math.PI*2/ue);w.uniforms.uNow.value=n,w.uniforms.uBreath.value=r,E.uniforms.uNow.value=n,E.uniforms.uBreath.value=r,B.uniforms.uNow.value=n,G&&(W.uniforms.uNow.value=n),O.uniforms.uNow.value=n,O.uniforms.uBreath.value=r,F.uniforms.uNow.value=n,t?(T.scale.setScalar(1),D.scale.setScalar(1),Se.scale.setScalar(1),I.scale.setScalar(1),S.position.set(0,0,7),S.lookAt(0,0,0)):(T.rotation.y=Y*.05,T.rotation.z=.14+.05*Math.sin(Y*.1),D.rotation.y=Y*.08,T.scale.setScalar(r),D.scale.setScalar(r),Se.scale.setScalar(r),I.scale.setScalar(r),Fe(Y),S.position.x=Math.sin(Y*.07)*.08,S.position.y=Math.cos(Y*.09)*.05,S.lookAt(0,0,0));let i=!1;for(let[,e]of J)e.morphTarget>.5&&(i=!0);let a=2*e;$+=Math.max(-a,Math.min(a,+!!i-$)),w.uniforms.uDim.value=$,E.uniforms.uDim.value=$,B.uniforms.uDim.value=$,G&&(W.uniforms.uDim.value=$),O.uniforms.uDim.value=$,F.uniforms.uDim.value=$;for(let e of K)e.active&&(e.line.material.opacity*=1-$*.85);for(let[r,i]of J){let a=(i.target>i.dissolve?1.6:.8)*e;i.dissolve+=Math.max(-a,Math.min(a,i.target-i.dissolve));let o=(i.morphTarget>i.morph?1.15:.95)*e;i.morph+=Math.max(-o,Math.min(o,i.morphTarget-i.morph)),i.mat.uniforms.uDissolve.value=i.dissolve,i.mat.uniforms.uMorph.value=i.morph,i.mat.uniforms.uNow.value=n,i.mat.uniforms.uDim.value=i.morphTarget>.5||i.morph>.01?0:$;let s=t?1:1+.06*Math.sin(Y*1.1+i.phase*6.28);i.mat.uniforms.uPulse.value=s;let{x:c,y:l}=Be(i.xPct,i.yPct);if(i.morphTarget>.5||i.morph>.01)i.points.position.set(c,l,v),i.points.rotation.y=i.frozenRot,i.points.scale.setScalar(1);else if(t)i.points.position.set(c,l,v),i.points.scale.setScalar(1);else{let e=Math.sin(Y*.85+i.phase*6.28)*.07,t=Math.cos(Y*.55+i.phase*4.2)*.04;i.points.position.set(c+t,l+e,v),i.points.rotation.y=Y*.12+i.phase,i.points.scale.setScalar(s)}i.removing&&i.dissolve>.98&&(b.remove(i.points),i.mat.dispose(),J.delete(r))}C.render(b,S)}Xe();let Ze=document.visibilityState===`visible`,Qe=!0;function $e(){let e=Ze&&Qe;e!==Q&&(e?(Q=!0,We.getElapsedTime(),Xe()):(Q=!1,cancelAnimationFrame(Z)))}x.current.setRunning=e=>{Qe=!!e,$e()};function et(){Ze=document.visibilityState===`visible`,$e()}return document.addEventListener(`visibilitychange`,et),()=>{Q=!1,cancelAnimationFrame(Z),document.removeEventListener(`visibilitychange`,et),C.domElement.removeEventListener(`pointerdown`,Ke),Je.disconnect();for(let[,e]of J)b.remove(e.points),e.mat.dispose();J.clear(),_e.dispose(),w.dispose(),be.dispose(),E.dispose(),xe.dispose(),O.dispose(),P.dispose(),F.dispose(),z.dispose(),B.dispose(),G&&(U.dispose(),W.dispose()),q.dispose();for(let e of K)e.line.geometry.dispose(),e.line.material.dispose();I.geometry.dispose(),I.material.dispose(),C.domElement.removeEventListener(`webglcontextlost`,he),C.domElement.removeEventListener(`webglcontextrestored`,ge),C.dispose(),oe(C),e.removeChild(C.domElement)}},[b]),(0,h.useEffect)(()=>{x.current.syncPlanets(e||[])},[e]),(0,g.jsx)(`div`,{ref:n,"aria-hidden":`true`,style:{position:`absolute`,inset:0,overflow:`hidden`,background:`#000`}})});export{b as default};