...
import {
  ...
  FQ_DRIFT_TIME_MULT,
  FQ_DUAL_TIME_MULT,
  // The honest model — THE model since 2026-09-20, no longer a shadow report.
...
/* -Infinity, not Infinity: the invariant flipped from "generosity must not
   rise" to "difficulty must not fall", so the seed has to be below everything
...
for (let lv = 1; lv <= FQ_LADDER_LEVELS; lv += 1) {
  ...
  for (let w = 0; w < waves; w += 1) {
    ...
    prevPerTarget = perTarget;
    prevWaveTc = want.tc;
    lastRealised = fqLadderDifficultyLogit(lv);
  }
  /*
   ...
   *
   * ⚠ THE INVARIANT IS DIFFICULTY, NOT GENEROSITY (changed 2026-09-20, when the
   * clock moved onto the honest model). It used to assert that the multiple of
   * expert pace never rose, which was a fair proxy while `per` barely moved
   * inside a tier — the superseded flat model priced every board in a tier
   * identically. It is not a fair proxy any more: the honest model prices a
   * 48-cell, 94%-same-hue board at more than twice a 20-cell pop-out, so a level
   * can hand back MORE spare time than the one before and still be harder,
   * because there is far more work to do in it. Measured: L38 grants 1.129x
   * against L37's 1.042x and is unambiguously the harder level.
   *
   * `fqLadderDifficultyLogit` carries both the tightness of the clock AND the
   * absolute load, and it is the same number the Elo placement reads — so the
   * gate and the ability model now argue about one quantity instead of two.
   */
  assert(
    lastRealised >= prevLastRealised - 1e-9,
    `ladder L${lv}: difficulty falls — b=${lastRealised.toFixed(3)} against `
      + `b=${prevLastRealised.toFixed(3)} at L${lv - 1}`,
  );
  prevLastRealised = lastRealised;
}

/* ═══════════════════════════════════════════════════════════════════════════
 * FEASIBILITY AGAINST THE HONEST MODEL — A GATE SINCE 2026-09-20.
 *
 * It started as a report, because the clock was generated by the superseded
 * flat model while this measured the honest one; failing would have failed
 * shipped, playable content, and a gate that fails working code gets weakened
 * and then protects nothing. It reported 13 of 330 waves under expert pace.
 *
 * The clock is DERIVED from the honest model now, so the relationship is no
 * longer empirical: `time / (per·tc)` reduces to the wave headroom, and
 * `FQ_WAVE_HR_FLOOR` is above 1. A wave below 1.0x is therefore not a tuning
 * problem to argue about — it means the derivation in `fqLadderWaveTable`
 * broke, most likely the feasibility clamp that has to beat the monotone cap.
 * That is exactly the kind of thing worth failing a build over.
 * ═══════════════════════════════════════════════════════════════════════════ */
{
  const honest = [];
  for (let lv = 1; lv <= FQ_LADDER_LEVELS; lv += 1) {
    ...
    for (let w = 0; w < waves; w += 1) {
      ...
      const perHonest = expertTargetSecForBoard({
        cols: board.cols, rows: board.rows, cells, tc: shape.tc, interference, mech,
      });
      honest.push({
        lv, w: w + 1, ratio: shape.time / (perHonest * shape.tc), tc: shape.tc, time: shape.time,
      });
    }
  }
  const worst = honest.reduce((a, b) => (b.ratio < a.ratio ? b : a));
  const under = honest.filter((x) => x.ratio < 1);
  assert(
    under.length === 0,
    `ladder: ${under.length} wave(s) below expert pace, worst ${worst.ratio.toFixed(3)}x at `
      + `L${worst.lv} w${worst.w} (${worst.tc} targets, ${worst.time}s). The clock is derived `
      + 'from this model, so this should be impossible — check the feasibility clamp in '
      + 'fqLadderWaveTable, which must beat the cross-level monotone cap.',
  );
  console.log(
    `\n  honest model: worst wave ${worst.ratio.toFixed(3)}x expert at L${worst.lv} w${worst.w} `
    + `(${worst.tc} targets, ${worst.time}s); 0 of ${honest.length} waves under 1.0x\n`,
  );
}
