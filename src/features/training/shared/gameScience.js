...
export const GAME_SCIENCE = {
  ...
  'task-switch': {
    en: {
      title: 'The science',
      intro: 'A cue names the feature to answer on, then a coloured shape appears. The two keys never move — left is red or circle, right is blue or square — so the same picture needs different answers depending on which rule is live. This is cued task switching (Rogers & Monsell, 1995).',
      sections: [
        { h: '🔀 Switch cost', b: 'You are reliably slower on a trial where the rule just changed than on one where it repeated. That difference is the measure: it is the time your brain spends reconfiguring, and it shrinks with preparation.' },
        { h: '⏱️ Preparation', b: 'The gap between the cue and the shape is how long you get to get ready. Given about a second the cost nearly vanishes; given a fraction of one it does not, which is why the gap shortens as you improve.' },
        { h: '⚔️ Congruency', b: 'A red circle wants the same key under either rule; a red square does not. If you are slower on the conflicting ones, the feature you were told to ignore reached you anyway.' },
        { h: '📊 What we track', b: 'Accuracy, switch cost in milliseconds, and the congruency effect.' },
      ],
      foot: 'Read the cue before you look at the shape. This trains and tracks flexibility; it is not a medical test.',
    },
    ar: {
      title: 'العلم وراء اللعبة',
      intro: 'يظهر تلميح يحدّد السمة التي تجيب عنها، ثم يظهر شكل ملوّن. المفتاحان لا يتحرّكان — اليسار أحمر أو دائرة، واليمين أزرق أو مربّع — فالصورة نفسها تحتاج إجابتين مختلفتين حسب القاعدة الفعّالة. هذا اختبار تبديل المهمة (روجرز ومونسِل، ١٩٩٥).',
      sections: [
        { h: '🔀 كلفة التبديل', b: 'أنت أبطأ في المحاولة التي تغيّرت فيها القاعدة عنها حين تتكرّر. هذا الفارق هو المقياس: زمن إعادة الضبط في دماغك، ويقلّ مع الاستعداد.' },
        { h: '⏱️ الاستعداد', b: 'الفجوة بين التلميح والشكل هي مهلتك للاستعداد. بثانية تقريباً تكاد الكلفة تختفي، وبجزء منها لا تختفي — لذلك تقصر الفجوة كلما تحسّنت.' },
        { h: '⚔️ التوافق', b: 'الدائرة الحمراء تريد المفتاح نفسه تحت أيّ قاعدة، أمّا المربّع الأحمر فلا. إن كنت أبطأ في المتعارضة فقد وصلتك السمة التي طُلب منك تجاهلها.' },
        { h: '📊 ما نقيسه', b: 'الدقة، وكلفة التبديل بالملي ثانية، وأثر التوافق.' },
      ],
      foot: 'اقرأ التلميح قبل النظر إلى الشكل. هذه اللعبة تدرّب وتتابع المرونة؛ وليست اختباراً طبياً.',
    },
  },
  'sort-shift': {
    en: {
      title: 'The science',
      intro: 'Six cards, split three and three — then split the same six a completely different way, and again. This is the free-sorting condition of the D-KEFS Sorting Test (Delis, Kaplan & Kramer, 2001): where most card tasks ask you to discover an examiner’s rule, this asks you to generate your own.',
      sections: [
        { h: '💡 Generative flexibility', b: 'Producing rules is a different demand from detecting them. The first split is the one you were already looking at; the third means abandoning an idea that just worked.' },
        { h: '🔒 Set loss', b: 'Once you have sorted by colour, colour keeps presenting itself. Getting past it is the same disengagement that set-shifting tasks measure, without a hidden rule to guess.' },
        { h: '🗂️ Concept formation', b: 'Naming what a group has in common — perceptual for colour and shape, conceptual for what the thing is — engages the same describing-your-own-reasoning demand the clinical test scores separately.' },
        { h: '📊 What we track', b: 'Rules found per set, sets cleared, and how far you get before the obvious dimensions run out.' },
      ],
      foot: 'When you are stuck, ignore what the cards look like and ask what they are. This trains and tracks flexibility; it is not a medical test.',
    },
    ar: {
      title: 'العلم وراء اللعبة',
      intro: 'ست بطاقات تُقسم ثلاثاً وثلاثاً — ثم تقسم البطاقات نفسها بطريقة مختلفة تماماً، ثم مرة أخرى. هذا شرط الفرز الحر من اختبار D-KEFS للفرز (ديليس وكابلان وكرامر، ٢٠٠١): معظم مهام البطاقات تطلب اكتشاف قاعدة الفاحص، وهذه تطلب أن تولّد قواعدك أنت.',
      sections: [
        { h: '💡 المرونة التوليدية', b: 'توليد القواعد مطلبٌ مختلف عن كشفها. التقسيم الأول هو ما كنت تنظر إليه أصلاً، أمّا الثالث فيعني التخلّي عن فكرة نجحت للتوّ.' },
        { h: '🔒 التمسّك بالطقم', b: 'بعد أن تفرز حسب اللون يظلّ اللون يفرض نفسه. تجاوزه هو الانفكاك نفسه الذي تقيسه مهام تبديل الطقم، لكن دون قاعدة خفية تُخمَّن.' },
        { h: '🗂️ تكوين المفاهيم', b: 'تسمية ما يجمع المجموعة — إدراكي في اللون والشكل، مفاهيمي في ماهية الشيء — يستدعي المطلب ذاته الذي يقيسه الاختبار السريري بشكل منفصل.' },
        { h: '📊 ما نقيسه', b: 'القواعد المكتشفة في كل مجموعة، والمجموعات المكتملة، ومدى تقدّمك بعد نفاد الأبعاد الظاهرة.' },
      ],
      foot: 'حين تتعثّر، تجاهل شكل البطاقات واسأل ما هي. هذه اللعبة تدرّب وتتابع المرونة؛ وليست اختباراً طبياً.',
    },
  },
  'math-gates': {
    en: {
