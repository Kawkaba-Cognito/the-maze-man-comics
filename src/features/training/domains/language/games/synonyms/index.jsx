 ...
 * restrained accent. No per-question timer — answer at your own pace; harder
 * tiers still mix trickier formats. Survival mode keeps its own 60s run clock
 * (the top progress bar) since that's the only thing that ends a run.
 * Plays over a universe backdrop — word/option cards stay opaque and keep
 * their own ink-on-cream contrast; only the bare text (prompt/badge/header)
 * switches to cream-on-space.
 */
...
export function buildTrial({ mode, diff, level, trialNum, rng, isAr, ramp = 0 }) {
  ...
  if (kind === 'odd') {
    const trial = { ...genOdd(rng, isAr, tier), tier };
  ...
  if (raw.kind === 'similarity') {
    ...
    return { kind: 'similarity', tier, rel, prompt: isAr ? 'ما وجه الشبه؟' : 'What is the best link?', left: L(raw.a), right: L(raw.b), options: opts.map((o, i) => ({ key: i, ...o })) };
  }
  if (raw.kind === 'analogy') {
    ...
    markSeen(raw.id, persist);
    return { kind: 'analogy', tier, rel, prompt: isAr ? 'أكمل القياس' : 'Complete the analogy', stem: [L(a), L(b), L(c)], options: opts.map((o, i) => ({ key: i, ...o })) };
  }
...
export function WordLinksEngine({ mode, diff, level, seed, attempt, onResult, onExit, isAr, playSfx, awardPoints, awardFreeRun, cosmos = false }) {
  ...
  // Survival's own 60s run clock — the only thing that ends a run, since
  // questions themselves are no longer timed.
  useEffect(() => {
    if (!isSurvival) return undefined;
    let raf = 0;
    const tick = () => {
