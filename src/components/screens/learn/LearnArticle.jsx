...
function Block({ block, chrome, isAr, playSfx }) {
  switch (block.type) {
    ...
    case 'h2':
      return (
        <h2 style={{ fontFamily: "'Outfit', sans-serif", fontSize: 19, margin: '26px 0 10px', color: chrome.text }}>
...
export default function LearnArticle({ topic, isAr, chrome, playSfx }) {
  ...
  return (
    <article style={{ textAlign: isAr ? 'right' : 'left' }}>
      ...
      <h1 style={{ fontFamily: "'Outfit', sans-serif", fontSize: 24, lineHeight: 1.25, margin: '0 0 18px', color: chrome.text }}>
        {title}
      </h1>
